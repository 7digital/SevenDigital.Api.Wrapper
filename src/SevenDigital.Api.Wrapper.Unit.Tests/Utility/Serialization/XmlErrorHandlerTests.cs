...
namespace SevenDigital.Api.Wrapper.Unit.Tests.Utility.Serialization
{
	...
	public class XmlErrorHandlerTests
	{
		...
		{
			const string invalidXml = "ERROR--ERROR!";
			var xmlErrorHandler = new XmlErrorHandler();

			XElement responseAsXml = xmlErrorHandler.GetResponseAsXml(invalidXml);

			Assert.That(responseAsXml.Name.LocalName, Is.EqualTo("response"));
		}

		[Test]
		public void Should_throw_parse_exception_with_html()
		{
			// Some server error responses are not valid xml at all, even when wrapped in an <error> tag
			// E.g. tags like "<hr>" and "<br>" cannot be parsed as xml as they do not close properly. 
			const string invalidXml = "<!DOCTYPE html><html><body><p>Some<br>Text<hr></p></body></html>";

			var xmlErrorHandler = new XmlErrorHandler();

			Assert.Throws<ApiXmlException>(() => xmlErrorHandler.GetResponseAsXml(invalidXml));
		}
